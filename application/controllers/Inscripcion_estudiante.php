...
class Inscripcion_estudiante extends CI_Controller
{
      ...
      $primer_apellido = mb_strtoupper($this->input->post("primer-apellido"));
       ...
       if($existe==0){
           ...
           if($filas!=-1){
               ...
               if ($this->enviar_correo_activacion($correo,$nombres." ".$primer_apellido." ".$segundo_apellido,$codigo_activacion)){
   ...
   function  activar($codigo_activacion=""){
    ...
    function enviar_correo_activacion($correo,$estudiante ,$codigo_activacion){

        $this->load->library('correos');

        return $this->correos->activacion_de_cuenta("Registro de estudiante",$correo,$estudiante ,$codigo_activacion);
